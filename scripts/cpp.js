...
function page_open(id) {
  ...
  document.getElementsByClassName("topic")[id].style.backgroundImage =
    "linear-gradient( #3a1c71,  #3a1c71)";

  document.getElementsByClassName("topic")[id].scrollIntoView();

  if (parseInt(localStorage.getItem("last_location")) == 0) {
    document.getElementsByClassName("nav_button")[0].style.visibility =
      "hidden";
  } else {
    document.getElementsByClassName("nav_button")[0].style.visibility =
      "visible";
  ...
  if (
    parseInt(localStorage.getItem("last_location")) + 1 ==
    document.getElementById("side_nav").childElementCount
  ) {
    document.getElementsByClassName("nav_button")[1].style.visibility =
      "hidden";
  } else {
    document.getElementsByClassName("nav_button")[1].style.visibility =
      "visible";
...
function next_btn() {
  ...
  if (
    parseInt(localStorage.getItem("last_location")) + 1 <
    document.getElementById("side_nav").childElementCount - 1
  ) {
    page_open(parseInt(localStorage.getItem("last_location")) + 1);
  } else if (
    parseInt(localStorage.getItem("last_location")) + 1 ==
